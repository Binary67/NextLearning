...
vi.mock("@/lib/learning-state", () => ({
  ...
  parseLearningSessionInput: mocks.parseLearningSessionInput,
  recordLearningSession: vi.fn(),
  validateSessionReferences: vi.fn(),
}));
...
describe("learning-session route", () => {
  ...
  it("accepts a valid session exactly at the 100 KiB limit", async () => {
    ...
    expect(mocks.parseLearningSessionInput).toHaveBeenCalledOnce();
  ...
  it.each([
    ...
    async (_description, contentLength) => {
      ...
      expect(mocks.parseLearningSessionInput).not.toHaveBeenCalled();
    },
  );
...
function createRequest(body: string, contentLength?: string) {
  return new Request(
    `http://localhost/api/tutorials/${tutorialId}/learning-state/sessions`,
    {
      method: "POST",
      headers: {
